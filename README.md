...
It does produce the output you asked for but not sure how brittle it is. I think there's small bug in that the paragraph containers get embedded inside each other which doesn't seem to affect the output but is next on my list to fix but I've gone over the 4 hour time limit suggested so might submit this as is
